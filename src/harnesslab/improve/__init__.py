...
Public API:
    fingerprint_for_event(event)            -> (kind, signature) | None
    fingerprint_for_eval_failure(name, msg) -> (kind, signature)
    build_clusters(events, eval_results, min_occurrences=2) -> list[Cluster]
    generate(events, eval_results, min_occurrences, now=None) -> list[Proposal]
