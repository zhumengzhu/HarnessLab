...
reply with a simple substring check (``final_reply_contains``) that survives
output variation. It must never be wired into the deterministic eval/replay
paths.
...
from harnesslab.eval.runner import _build_tool_registry, _drive_turns, _limits_for_task
...
def passes_task(task: Task, final_reply: str) -> bool:
    """A task passes when every ``final_reply_contains`` substring appears.

    Substring presence is the one correctness signal robust to a live model's
    non-deterministic phrasing; span-order expectations are intentionally not
    checked here (that is the deterministic ``eval`` runner's job).
    """

    return all(needle in final_reply for needle in task.expected.final_reply_contains)


class PromptBenchmark:
    """Score prompt candidates by running the loop against a task suite."""
    ...
    def run(self, candidate: PromptCandidate) -> BenchmarkResult:
        ...
        for _ in range(self._repeats):
            for task in self._suite.tasks:
                final_reply = self._drive(candidate, task)
                trials += 1
                if passes_task(task, final_reply):
                    passes += 1
        return BenchmarkResult(candidate_id=candidate.id, passes=passes, trials=trials)

    def _drive(self, candidate: PromptCandidate, task: Task) -> str:
        with tempfile.TemporaryDirectory() as ws:
            ...
            loop = HarnessLoop(
                ...
                policy=DefaultPolicy(
                    ...
                    shell_profile=(
                        ...
                        and task.policy.shell_profile is not None
                        else None
                    ),
            ...
            replies = _drive_turns(
                loop, session.id, task.turns, default_goal=task.goal
            )
